                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3">Tempat Praktik</th>
                            ...
                            <tbody>
                                @forelse($dokters as $dokter)
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4">{{ $dokter->praktik ? $dokter->praktik->nama_praktik : 'Belum ada praktik' }}</td>
                                        <td class="px-6 py-4">{{ $dokter->spesialis }}</td>
                                        <td class="px-6 py-4">{{ $dokter->email }}</td>
                                        <td class="px-6 py-4">{{ $dokter->no_telpon }}</td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center space-x-4">
                                                ...
                                                <form action="{{ route('admin.dokter.destroy', $dokter) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
